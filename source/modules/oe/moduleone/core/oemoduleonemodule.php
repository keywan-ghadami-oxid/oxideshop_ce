...
class oeModuleOneModule extends oxModule
{
    ...
    {
        $sModuleId = 'oemoduleone';

        $this->setModuleData(
            array(
                'id'          => $sModuleId,
                'title'       => 'OE Module One',
                'description' => 'OE Module One Module',
            )
        );

        $this->load($sModuleId);

        oxRegistry::set('oeModuleOneModule', $this);
    }
    ...
    {
        if ($blUseModulePrefix) {
            $sModuleSettingName = 'oeModuleOne' . (string) $sModuleSettingName;
        }
    ...
    {
        return oxRegistry::getConfig()->getModulesDir() . 'oe/moduleone/';
    }
}
